...
async fn create(
    ...
    match form_data {
        Ok(mut fd) => {
            let file_field = fd.files.get("file");
            let ext_field = fd.texts.remove("ext");
                ...
                if let Some(file_fields) = file_field {
                    ...
                    let encoded_name = base64::encode_config(
                        format!(
                            ...
                            original_name =
                                f_file_name.as_ref().unwrap_or(&String::from("unknown"))[0..10]
                                    .to_string(),
                    ...
                    let new_file_path =
                        PathBuf::from(format!("storage/{}.{}", &encoded_name, &ext));
                    fs::copy(f_path, new_file_path)
                        .expect("Failed to copy file into `storage/` folder");
